...
def main(name, file_time, target):
    global mil
    mil = "0."
    global base_time
    base_time = file_time
    ...
    # カメラ準備 ###############################################################
    cap = cv.VideoCapture(name + "_1.mp4")
    ...
    input_fps = int(cap.get(cv.CAP_PROP_FPS))

    # 出力動画の設定
    filename = name + "_motion" + str(target) + ".mp4"
    fourcc = cv.VideoWriter_fourcc(*'mp4v')  # 使用するコーデック
    out = cv.VideoWriter(filename, fourcc, input_fps, (960, 540))
    
    # 座標格納ファイル
    global file
    file = open(name + "_" + str(target) + ".txt", "w")
    num_frame = 0
    firstTime = True
    ...
    while True:
        ...
        if firstTime:
            if target == 1:
                image = cv.rectangle(image, (0,0), (480, 540), (0,0,0), -1)
            elif target == 2:
                image = cv.rectangle(image, (480,0), (960, 540), (0,0,0), -1)
            firstTime = False
...
def draw_landmarks(
    ...
    # UNIX時間で取得
    point_time = base_time + ((num_frame-1) * (1/input_fps))
    # text = datetime.datetime.fromtimestamp(point_time)
    text = str(point_time)
    ...
    for index, landmark in enumerate(landmarks.landmark):
        ...
        if index == 0:  # 鼻
            ...
            text += "," + str(index) + "," + str(landmark_x) + "," + str(landmark_y)
        if index == 1:  # 右目：目頭
            #cv.circle(image, (landmark_x, landmark_y), 5, (0, 255, 0), 2)
            text += "," + str(index) + "," + str(landmark_x) + "," + str(landmark_y)
        if index == 2:  # 右目：瞳
